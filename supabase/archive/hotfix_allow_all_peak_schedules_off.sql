-- 피크 스케줄을 전부 꺼도 저장 가능하도록 서버 제약 완화 (Dashboard → SQL Editor → Run)
--
-- 증상: 어드민에서 점심/저녁 피크 알림을 모두 OFF로 끄고 저장하면
--       "활성화된 피크 스케줄이 최소 1개 필요해요" 예외로 저장이 거부됨.
-- 판단: 이미 peak_fcm_enabled(서버 FCM 전체 스위치)가 따로 있는데, 그와 별개로
--       개별 스케줄을 전부 끄는 것까지 막을 이유가 없음 — 설계 실수로 판단해 제거.
--       스케줄 "목록"이 비어있는 것(0개)은 계속 막는다 (1~8개 범위 제약 유지).
