...
| 탭 | 기능 |
|---|---|
| **핵심 지표** | DAU/MAU, 제보·클릭률·푸시 KPI, 차트, 오너 등록 현황, 혼잡도 계산(사장님 영향력), 엑셀(.xlsx) 내보내기 |
| **매장 관리** | 검색, 추가/수정/삭제, 최근 제보, 사장님 코드 발급 |
| **인기 관리** | 알고리즘/수동 순위 토글, 수동 순위 조정 |
| **지도 등록** | 카카오 로컬 검색 → DB 등록 + 6자리 인증번호 |
...
| `VITE_GOOGLE_MAPS_API_KEY` / `GOOGLE_MAPS_API_KEY` | 영업시간·사진 보강 (Google Places) |
| `VITE_KAKAO_REST_API_KEY` / `KAKAO_REST_API_KEY` | 지도 등록 탭 장소 검색 (필수) |
| `VITE_KAKAO_JAVASCRIPT_KEY` / `KAKAO_JAVASCRIPT_KEY` | 카카오맵 링크 (선택) |
