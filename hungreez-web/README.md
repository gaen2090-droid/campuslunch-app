...
제품명: Campus Lunch(캠퍼스런치) 등 · 상호: 헝그리즈(Hungreez)
Hungreez는 여러 모바일 앱을 기획·개발·운영하는 팀이며, Campus Lunch는 현재 출시·운영 중인 제품입니다.
...
2. **도메인 이름이 조직과 연관**될 것 (예: `hungreez.kr`, `hungreez.co.kr`)  
...
2. 이 저장소를 연결하고 **Root Directory**를 `hungreez-web`으로 설정  
...
## 커스텀 도메인: hungreez.site

1. Vercel에 `hungreez-web` 배포  
2. Project → **Settings → Domains** → `hungreez.site` / `www.hungreez.site` 추가  
3. 도메인 등록처(DNS)에 Vercel이 안내하는 **A / CNAME** 기록  
4. `https://hungreez.site` 가 회사 소개 페이지로 열리는지 확인  
5. **`hungreez@hungreez.site`** 메일 개설 (Google Workspace / 가비아 메일 / ImprovMX 등)  
   → Apple 조직 등록 시 업무 이메일로 사용

## 체크리스트 (제출 전)

- [ ] `https://hungreez.site` 가 브라우저에서 열림 (주차 페이지 아님)
- [ ] Hungreez / 헝그리즈 / Campus Lunch / 사업자정보가 보임
- [ ] `hungreez@hungreez.site` 수신 가능
