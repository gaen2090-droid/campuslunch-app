...
"""캠퍼스런치 법적 문서 PDF 생성 (개인정보처리방침·이용약관·운영정책·리워드).
...
def _privacy(pdf: LegalPDF) -> None:
    ...
    pdf.note_box(
        "Firebase Analytics·FCM은 현재 사용하지 않습니다. "
        "푸시는 flutter_local_notifications 기반 로컬 알림입니다."
    )
...
def _terms(pdf: LegalPDF) -> None:
    ...
    sections = [
        (
            "제1조 (목적)",
            "중앙대 인근 식당 혼잡도·제보·리워드 모바일 서비스 이용에 관한 권리·의무를 규정합니다.",
        ),
        (
            "제2조 (정의)",
            "서비스, 회원, 제보, 스탬프, 기프티콘(쿠폰)의 정의를 둡니다.",
        ),
        (
            "제3조 (효력·변경)",
        ...
        (
            "제5조 (서비스)",
            "매장 조회, 지도, 제보, 북마크, 스탬프·쿠폰, 로컬 추천 알림, 피드백.",
        ),
        (
            "제6조 (혼잡도)",
        ...
        (
            "제7조 (회원 의무)",
            "허위·도배 제보, 욕설·광고, 부정 리워드, 시스템 악용 금지.",
        ),
        (
            "제8조 (리워드)",
        ...
        (
            "제9조 (게시물)",
            "제보 책임은 회원. 위반 시 삭제·제한.",
        ),
        (
            "제10조 (이용 제한)",
            "위반 시 제보·리워드·계정 제한. 탈퇴 가능.",
        ),
        (
            "제11조 (변경·중단)",
    ...
    for title, body in sections:
        ...
        pdf.body(body)
    pdf.ln(2)
    pdf.body(f"부칙 — 본 약관은 {EFFECTIVE}부터 시행합니다.")
