...
class KakaoMapController: NSObject, FlutterPlatformView, MapControllerDelegate, GuiEventDelegate, KakaoMapEventDelegate {
    ...
    private func onMethodCall(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "setRoutePolyline":
            setRoutePolyline(call, result)
        case "clearRoutePolyline":
            clearRoutePolyline(call, result)
        default:
    ...
    // MARK: - Route polyline (OSRM 등 외부 경로 좌표)
    private let kRouteLayerId = "campus_route_layer"
    private let kRouteShapeId = "campus_route_shape"
    private let kRouteStyleSetId = "campus_route_style_set"

    private func setRoutePolyline(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        guard let args = call.arguments as? [String: Any],
              let pointsArray = args["points"] as? [[String: Any]] else {
            result(FlutterError(code: "E001", message: "Invalid arguments for setRoutePolyline", details: nil))
            return
        }

        withKakaoMapView(result) { view in
            let manager = view.getShapeManager()
            if manager.getShapeLayer(layerID: self.kRouteLayerId) == nil {
                _ = manager.addShapeLayer(layerID: self.kRouteLayerId, zOrder: 5000)
            }
            guard let layer = manager.getShapeLayer(layerID: self.kRouteLayerId) else {
                result(FlutterError(code: "E002", message: "ShapeLayer not found", details: nil))
                return
            }
            ...
            if !self.registeredRouteStyleSetIds.contains(styleSetId) {
                ...
                manager.addPolylineStyleSet(PolylineStyleSet(
                    styleSetID: styleSetId,
                    styles: [polylineStyle],
                    capType: .round
                ))
                self.registeredRouteStyleSetIds.insert(styleSetId)
            }

            layer.removeMapPolylineShape(shapeID: self.kRouteShapeId)

            var mapPoints: [MapPoint] = []
            for p in pointsArray {
                guard let lat = p["latitude"] as? Double,
                      let lng = p["longitude"] as? Double,
                      lat.isFinite, lng.isFinite else { continue }
                mapPoints.append(MapPoint(longitude: lng, latitude: lat))
            }
            ...
            let options = MapPolylineShapeOptions(
                shapeID: self.kRouteShapeId,
                styleID: styleSetId,
                zOrder: 1
            )
            options.polylines = [
                MapPolyline(line: mapPoints, styleIndex: 0),
            ]
    ...
    private func clearRoutePolyline(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        withKakaoMapView(result) { view in
            view.getShapeManager()
                .getShapeLayer(layerID: self.kRouteLayerId)?
                .removeMapPolylineShape(shapeID: self.kRouteShapeId)
            result(nil)
        }
    }
}
