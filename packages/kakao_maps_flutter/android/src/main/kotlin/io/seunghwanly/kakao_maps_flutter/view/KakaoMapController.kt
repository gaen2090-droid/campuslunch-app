...
class KakaoMapController(
    ...
    private val routePolylineId = "campus_route_shape"
    ...
    private fun setRoutePolyline(args: JSONObject, result: MethodChannel.Result) {
        if (!::kMap.isInitialized) {
            result.error("E000", "Map not ready", null)
            return
        }

        val pointsArray = args.optJSONArray("points")
        ...
        for (i in 0 until pointsArray.length()) {
            ...
            val lng = point.optDouble("longitude", Double.NaN)
            if (!lat.isFinite() || !lng.isFinite()) continue
            latLngList.add(LatLng.from(lat, lng))
        }
        ...
        val shapeManager = kMap.shapeManager
        if (shapeManager == null) {
            result.error("E002", "ShapeManager not found", null)
            return
        }

        val layer: ShapeLayer = shapeManager.layer
            ?: shapeManager.getLayer()
            ?: run {
                result.error("E002", "ShapeLayer not found", null)
                return
            }

        routePolyline?.let { existing ->
            layer.remove(existing)
            routePolyline = null
        }
        layer.getPolyline(routePolylineId)?.let { existing ->
            layer.remove(existing)
        }
        ...
        routePolyline = layer.addPolyline(options)
        result.success(true)
    }

    private fun clearRoutePolyline(result: MethodChannel.Result) {
        if (!::kMap.isInitialized) {
            result.success(null)
            return
        }
        ...
        val shapeManager = kMap.shapeManager
        if (shapeManager != null) {
            val layer = shapeManager.layer ?: shapeManager.getLayer()
            if (layer != null) {
                routePolyline?.let { layer.remove(it) }
                layer.getPolyline(routePolylineId)?.let { layer.remove(it) }
            }
        }
        routePolyline = null
        result.success(null)
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        ...
        when (call.method) {
            ...
            "setRoutePolyline" -> setRoutePolyline(asJSONObject(call.arguments), result)
            "clearRoutePolyline" -> clearRoutePolyline(result)
